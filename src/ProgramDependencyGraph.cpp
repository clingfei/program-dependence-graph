...
void pdg::ProgramDependencyGraph::dumpGlobal() {
  // std::ofstream output_file("addrVars_for_paratree");
  // std::ofstream output_file("global_list_funcs");
  outs() << _module->getGlobalList().size() << " global variables in total\n";
  for (auto &global_var : _module->getGlobalList()) {
    ...
    // global_var.print(stream);
    outs() << "================ global: "  << "==============\n";
    for (User* user : global_var.users()) {
      Node* n = _PDG->getNode(*user);
      user->print(outs());
      outs() << "\n";
      if (n == nullptr) {
        //errs() << "user not found\n";
